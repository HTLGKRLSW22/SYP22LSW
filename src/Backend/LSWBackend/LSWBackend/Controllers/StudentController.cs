...
namespace LSWBackend.Controllers
{
    ...
    public class StudentController : ControllerBase
    {
        ...
        public ActionResult<StudentDto> GetStudentOffers() {
            int userId = Int32.Parse(User.Claims.Single(x => x.Type == ClaimTypes.NameIdentifier).Value);
            string errorMsg = "";
            return Ok(new StudentDto().CopyPropertiesFrom(_service.GetOffersOfStudents(out errorMsg, userId)));
